...
public class ApplicationTestCase extends AbstractEmbeddedTestCase {
  ...
  public void applicationWithConnector() throws Exception {
    doWithinApplication(HTTP_ECHO, getAppFolder(HTTP_ECHO), embeddedTestHelper.createRetryTestOperation(port -> assertTestMessage(port)));
  }
  ...
  // }
  //
  // @Description("Embedded can be restarted, start an instance of the container, runs the test, stop it and start it again and
  // runs the test again")
  // @Test
  ...
  //
  // @Test
  // @Description("Even if 2 apps have the same name and were created at the same time, if one of them have different config
  // files, redeployment should be triggered")
